...
- The M9-BP5-CTRIP-U1 CTRIP Browser Acquisition Bounded Offer Probe Specification V0.1 is the formal CTRIP-specific Implementation Unit Authority under M9 Track B. It authorizes future bounded, opt-in, read-only CTRIP Browser probe implementation through normal public consumer paths, including one controlled booking/offer-expansion action to discover provider-side raw Level-2 Shopping Offer Evidence when directly observable. CTRIP-U1 implementation status: PLANNED / NOT IMPLEMENTED. It does not complete M9, does not select CTRIP as First Provider, does not authorize a formal CTRIP Adapter/Mapper, canonical Domain Offer mapping, final PurchaseAccess implementation, direct private API client, purchase action, login/challenge bypass, Domain/Provider Port/Public API/database/frontend change, CI dependency on CTRIP, or Contract Amendment.
