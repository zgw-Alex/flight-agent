...
- The M9-BP5-U1 FLIGGY Browser Acquisition Read-Only Probe Specification V0.1 is the currently authorized first M9 implementation unit. It authorizes a bounded, opt-in, read-only Browser acquisition probe for FLIGGY Level-1 provider-side raw evidence only; it does not implement the formal Fliggy FlightProvider Adapter/Mapper, Level-2 offer crawl, final PurchaseAccess, or canonical Domain mapping. The M9 Browser Acquisition Provider Exploration & Collaboration Playbook V1.0 is supporting methodology/governance for multi-provider Track B work. The FLIGGY Browser Pilot B-P1 through B-P4 V0.2 is live experiment/supporting evidence, not stable Domain/Provider contract authority.
