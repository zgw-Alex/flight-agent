...
- M8-U6H-CA04 is the accepted additive Contract Amendment / Implementation Specification Authority for bounded Preference Importance semantic expansion in M8 Hybrid Parser/Patch. CA04 status: ACCEPTED / IMPLEMENTATION READY / NOT IMPLEMENTED. It authorizes only bounded importance semantics for existing `PRICE` and `FEWER_STOPS` soft preferences, using canonical `LOW`, `MEDIUM`, `HIGH`, plus Specification-defined `null` legacy fallback semantics. CA04 preserves DeepSeek as evidence-closed, schema-constrained, non-authoritative semantic proposal capability; preserves M3 Requirement authority, M6 ranking authority/weights/formula, and M7 impact/execution authority; and does not authorize `DEPARTURE_TIME`, `ARRIVAL_TIME`, `AIRPORT_MATCH` ranking expansion, conditional tradeoff IR, free numeric weights, or direct LLM commit authority. Implementation is authorized only according to CA04's staged Implementation Units.
- M8-U6H-CA04-CA01 is the accepted narrow additive Contract Amendment Authority for explicit no-preference / preference-removal representation under CA04 Patch resume. CA01 authorizes only the non-authoritative resolver relation `REMOVE_SOFT_PREFERENCE` with target allowlist exactly `PRICE` or `FEWER_STOPS`, `value=null`, `importance=null`, explicit evidence closure, and confidence as diagnostic metadata only. Deterministic Patch builder remains responsible for mapping validated removal intent through existing M3 same-scope targeting: unique existing same-scope preference maps to `REMOVE_PREFERENCE`, absent target preference maps to `NO_OP`, and ambiguous state remains reject/ambiguous with no guessing. CA01 does not make `importance=None`, `LOW`, or low confidence removal authority; does not affect hard `MAX_PRICE` / `MAX_STOPS`; preserves M3 Requirement authority, M6 ranking weights/formula, and M7 stale/impact/execution authority; and does not authorize Prompt V3, live DeepSeek behavior, new ranking scopes, free numeric weights, conditional tradeoff, utility IR, or direct LLM commit authority. CA04-U3 is authorized to resume only after this governance intake and remains not implemented by this intake.
