...
4.  **当前 Milestone / Implementation Unit 已确认规格**
    ...
    -   `03_implementation-roadmap/milestones/M9/机票筛选Agent_M9_CTRIP_Candidate_Integration_Unit_Specification_V1.0.docx`
        ...
        -   Unit status：AUTHORIZED / READY FOR IMPLEMENTATION；上游 Audit 结论为 PASS WITH LOCAL INTEGRATION GAP，且已接受 Existing CandidateMerger / CandidateSnapshot semantics sufficient、single-provider CTRIP input supported、provider-local integration only sufficient、redundant CTRIP-specific merger/snapshot semantics not needed。
        -   仅授权 CTRIP single-provider candidate wiring：把 CTRIP canonical normalization results 以 provider-local / application wiring 方式交给 existing CandidateMerger 与 existing CandidateSnapshotAssembler。
        -   不授权 shared merger/snapshot semantic change、FLIGGY + CTRIP multi-provider aggregation、Ranking、Recommendation、Purchase Guidance、Public API、Domain/Provider Port/schema/frontend change、live CTRIP acquisition、anti-bot bypass、cookie/token/HAR persistence 或 M9 final Provider selection。
        -   Contract Amendment：NONE；下一步只允许执行独立 `M9-CTRIP-CANDIDATE-INTEGRATION-U1` implementation prompt。
...
M9-CTRIP-CANDIDATE-INTEGRATION-U1 CTRIP Candidate Pipeline Integration Unit Specification V1.0 已登记为 formal Implementation Unit Specification / Contract Authority。Unit status：AUTHORIZED / READY FOR IMPLEMENTATION；canonical path 为 `03_implementation-roadmap/milestones/M9/机票筛选Agent_M9_CTRIP_Candidate_Integration_Unit_Specification_V1.0.docx`。该 Unit 接受上游 Audit = PASS WITH LOCAL INTEGRATION GAP，确认 Existing CandidateMerger / CandidateSnapshot semantics sufficient、Provider-local integration only sufficient、Redundant CTRIP-specific merger/snapshot semantics needed = NO、Contract Amendment = NONE。该 Unit 仅授权 CTRIP single-provider candidate wiring，通过 provider-local / application wiring 复用 existing CandidateMerger 与 existing CandidateSnapshotAssembler；不授权 shared merger/snapshot semantic change、FLIGGY + CTRIP multi-provider aggregation、Ranking、Recommendation、Public API、Purchase Guidance、Domain/Provider Port/schema/frontend change、live CTRIP acquisition、anti-bot bypass、cookie/token/HAR persistence 或 M9 final Provider selection。
