...
4.  **当前 Milestone / Implementation Unit 已确认规格**
    ...
    -   `03_implementation-roadmap/milestones/M9/机票筛选Agent_M9_TONGCHENG_BROWSER_DIAG_U2_Implementation_Unit_Specification_V1.0.docx`
        -   M9 Track B 的正式 TONGCHENG provider-local Diagnostic Implementation Unit Authority。
        -   Unit ID：`M9-TONGCHENG-BROWSER-DIAG-U2`；status：AUTHORIZED / READY FOR DIAGNOSTIC IMPLEMENTATION。
        -   授权 provider-local instrumentation、challenge classification accuracy repair、stage/timing diagnostics、对应 provider-local tests/offline fixtures，以及最多 1 次 headed + 1 次 headless 的 bounded diagnostic rerun。
        -   U1 保持 BLOCKED；本 Unit 不将 U1 改为 PASS / CLOSED。
        -   记录 V0 不要求自动购买或稳定 final deep link；在可靠 Provider evidence 支持下，`SEARCH_INSTRUCTION` / `SEARCH_REPLAY` 可满足 V0 Actionability，由用户回到外部平台搜索/购买。
        -   不授权 booking/login automation、Canonical Mapper / Common Normalizer、FlightProvider Port、Domain、SearchPlan、Requirement、Patch、Ranking、Recommendation、Publication、Public API、shared PurchaseAccess semantics、CandidateSnapshot、aggregation、direct private endpoint client、cookie/session/token/HAR persistence、login/challenge bypass、main integration、M9 closure、final Provider selection 或 Contract Amendment；Shared Contract Convergence Gate 继续有效。
