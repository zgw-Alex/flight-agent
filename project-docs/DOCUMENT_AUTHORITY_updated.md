...
4.  **当前 Milestone / Implementation Unit 已确认规格**
    ...
    -   `03_implementation-roadmap/milestones/M9/机票筛选Agent_M9_CTRIP_Canonical_Entry_and_Common_Normalizer_Integration_Unit_Specification_V1.0.docx`
        -   M9-CTRIP-CANONICAL-U1 的正式 Implementation Unit Specification / Contract Authority。
        -   Unit status：AUTHORIZED / READY FOR IMPLEMENTATION；上游 Shared Authority 为 M9 Shared Canonical Mapping Specification V1.0；直接上游 CTRIP Mapper Authority 为 M9 CTRIP Evidence-to-Mapped Mapper Unit Specification V1.0。
        -   仅授权 CTRIP `Mapped → existing Common Normalizer → Canonical Entry / compatibility` 边界：复用现有 CommonNormalizer，建立薄 provider-local canonical-entry integration，验证 `CtripProviderMapper` 输出可诚实进入既有 canonical domain path。
        -   保持 `Evidence → Mapped → Canonical` 分层；Shared Contract 不因本接纳发生 Amendment；Contract Amendment：NONE。
        -   不授权 Candidate Merger、CandidateSnapshot、multi-provider aggregation、Ranking、Recommendation、Purchase Guidance、Public API、Domain/Provider Port/schema/frontend change、live CTRIP acquisition、anti-bot bypass、cookie/token/HAR persistence 或 M9 final Provider selection。下一步只允许执行独立 `M9-CTRIP-CANONICAL-U1` implementation prompt；下游共享集成仍未授权。
    -   `03_implementation-roadmap/milestones/M9/机票筛选Agent_M9_CTRIP_Candidate_Pipeline_Compatibility_Audit_Unit_Specification_V1.0.docx`
        -   M9-CTRIP-CANDIDATE-AUDIT-U1 的正式 Read-Only Compatibility Audit Unit Specification / Authority。
        -   Unit status：AUTHORIZED / READY FOR READ-ONLY AUDIT；上游 CTRIP Mapper 与 CTRIP Canonical Entry authority 已完成并继续作为本审计上游。
        -   仅授权对 existing M4 Candidate Merger / CandidateSnapshot path 是否兼容 CTRIP canonical outputs 进行只读审计；Audit execution 默认 NO SOURCE CHANGE / NO TEST CHANGE / NO FIXTURE CHANGE，且 NO COMMIT / NO PUSH。
        -   不授权 Candidate Merger implementation、CandidateSnapshot implementation/schema change、multi-provider aggregation、provider registry / Composition Root、Ranking、Recommendation、Purchase Guidance、Public API、DB/migration、CI gate / Golden output 或 Shared Canonical Mapping Specification 修改。
        -   Shared Contract Amendment：NONE；只有 read-only audit evidence 证明真实 shared gap 时，才进入 STOP / Human Decision / Contract Amendment 流程。
    -   `03_implementation-roadmap/milestones/M9/机票筛选Agent_M9_CTRIP_Candidate_Integration_Unit_Specification_V1.0.docx`
        -   M9-CTRIP-CANDIDATE-INTEGRATION-U1 的正式 Implementation Unit Specification / Contract Authority。
        -   Unit status：PASS / IMPLEMENTED at commit `0e5e052de5bc87207bb5d1760217dddb245f4bad`；上游 Audit 结论为 PASS WITH LOCAL INTEGRATION GAP，且已接受 Existing CandidateMerger / CandidateSnapshot semantics sufficient、single-provider CTRIP input supported、provider-local integration only sufficient、redundant CTRIP-specific merger/snapshot semantics not needed。
        -   仅授权并实现 CTRIP single-provider candidate wiring：把 CTRIP canonical normalization results 以 provider-local / application wiring 方式交给 existing CandidateMerger 与 existing CandidateSnapshotAssembler。
        -   未授权也未实现 shared merger/snapshot semantic change、FLIGGY + CTRIP multi-provider aggregation、Ranking、Recommendation、Purchase Guidance、Public API、Domain/Provider Port/schema/frontend change、live CTRIP acquisition、anti-bot bypass、cookie/token/HAR persistence 或 M9 final Provider selection。
        -   Contract Amendment：NONE。
    -   `03_implementation-roadmap/milestones/M9/机票筛选Agent_M9_CTRIP_Runtime_Integration_Audit_Unit_Specification_V1.0.docx`
        -   M9-CTRIP-RUNTIME-INTEGRATION-AUDIT-U1 的正式 Read-Only Runtime Integration Audit Unit Specification / Authority。
        -   Unit status：AUTHORIZED / READY FOR READ-ONLY AUDIT；上游 CTRIP Browser/acquisition、Mapper、Canonical Entry 与 Candidate Integration authority/evidence 已登记，Candidate Integration 已 PASS。
        -   仅授权对 runtime/application/provider wiring 进行 no-modification audit：SearchPlan → CTRIP provider execution、Browser acquisition / sanitized evidence handoff、mapper/canonical/candidate callable seams、provider registry/composition extension point 与 provider execution outcome semantics。
        -   Audit execution 默认 NO SOURCE CHANGE / NO TEST CHANGE / NO FIXTURE CHANGE / NO LIVE CTRIP；Audit report 属于 LOCAL-ONLY。
        -   不授权 runtime implementation、SearchPlan / FlightProvider Port / ProviderStatus / Execution semantic change、browser acquisition implementation change、registry/composition change、Candidate pipeline change、API/frontend/DB/CI change、live CTRIP call、anti-bot bypass 或 Contract Amendment。
        -   Contract Amendment：NONE unless later read-only audit evidence proves a real shared gap and enters STOP / Human Decision。
...
-   M9 One Real Flight Provider：OPEN / IN PROGRESS；formal implementation authority 为 `03_implementation-roadmap/milestones/M9/机票筛选Agent_第三阶段_M9_Real_Shopping_Provider_Implementation_Specification_V0.1.docx`。M9-BP5-U1 已实现，commit `7a29c4f2d2bd921d423551bc9caa3556b93acb83`。当前授权 FLIGGY diagnosis Unit 为 `03_implementation-roadmap/milestones/M9/机票筛选Agent_M9-BP5-U2_Fliggy_Browser_Probe_Live_Diagnosis_and_Repeatability_Specification_V0.1.docx`。M9-BP5-CTRIP-U1 CTRIP Browser Acquisition Bounded Offer Probe Specification 已完成 governance registration，Implementation Status：PLANNED / NOT IMPLEMENTED。M9 CTRIP Evidence-to-Mapped Mapper Unit 已完成 authority registration 与 mapper implementation evidence；M9-CTRIP-CANONICAL-U1 CTRIP Canonical Entry / Common Normalizer Integration Unit 已完成 governance registration 与 implementation，Unit status：PASS。M9-CTRIP-CANDIDATE-AUDIT-U1 CTRIP Candidate Pipeline Compatibility Audit Unit 已完成 governance registration；其上游审计结论 PASS WITH LOCAL INTEGRATION GAP 已接受。M9-CTRIP-CANDIDATE-INTEGRATION-U1 CTRIP Candidate Pipeline Integration Unit 已完成 governance registration 与 implementation，Unit status：PASS / IMPLEMENTED at commit `0e5e052de5bc87207bb5d1760217dddb245f4bad`，仅授权 CTRIP single-provider candidate wiring。M9-CTRIP-RUNTIME-INTEGRATION-AUDIT-U1 CTRIP Runtime Integration Compatibility Audit Unit 已完成 governance registration，Unit status：AUTHORIZED / READY FOR READ-ONLY AUDIT，不授权 runtime implementation 或 live CTRIP。M9-BP6-CA01 Lower-Bound Offer Price Semantics Proposal 已完成 human-approved governance registration，Decision Status：APPROVED，Implementation Status：NOT IMPLEMENTED；M9-BP6-CA01 Implementation Unit Breakdown and Specification 已完成 governance registration，CA01-U1～U6 状态为 PLANNED / NOT IMPLEMENTED。M9 closure 尚未声明；首个 Provider 尚未最终 SELECTED。
-   M9-CTRIP-RUNTIME-INTEGRATION-U1 CTRIP Runtime Integration Unit 已完成 governance registration，Unit status：AUTHORIZED / READY FOR IMPLEMENTATION。该 Unit 接受 Runtime Audit 结论 `PASS WITH LOCAL RUNTIME WIRING GAP`，仅授权 CTRIP provider-local FlightProvider execution adapter 与 additive bootstrap/provider registry/composition wiring；Bounded live revalidation remains separate future unit；Contract Amendment：NONE。
...
M9-CTRIP-CANONICAL-U1 CTRIP Canonical Entry / Common Normalizer Integration Unit Specification V1.0 已登记为 formal Implementation Unit Specification / Contract Authority。Unit status：AUTHORIZED / READY FOR IMPLEMENTATION；canonical path 为 `03_implementation-roadmap/milestones/M9/机票筛选Agent_M9_CTRIP_Canonical_Entry_and_Common_Normalizer_Integration_Unit_Specification_V1.0.docx`。该 Unit 以上游 M9 Shared Canonical Mapping Specification V1.0 与 M9 CTRIP Evidence-to-Mapped Mapper Unit Specification V1.0 为直接 Authority，只授权 CTRIP `Mapped → existing Common Normalizer → Canonical Entry / compatibility` 边界，复用既有 CommonNormalizer，保持 `Evidence → Mapped → Canonical` 分层，验证 `CtripProviderMapper` 输出进入既有 canonical domain path 的兼容性。Shared Contract preserved；Contract Amendment：NONE。该登记不授权 Candidate Merger、CandidateSnapshot、multi-provider aggregation、Ranking、Recommendation、Purchase Guidance、Public API、Domain/Provider Port/schema/frontend change、live CTRIP acquisition、anti-bot bypass、cookie/token/HAR persistence 或 M9 final Provider selection；下一步仅可在单独 implementation prompt 下执行本 Unit，实现完成后仍不得自动进入下游共享集成。

M9-CTRIP-CANDIDATE-AUDIT-U1 CTRIP Candidate Pipeline Compatibility Audit Unit Specification V1.0 已登记为 formal Read-Only Compatibility Audit Unit Specification / Authority。Unit status：AUTHORIZED / READY FOR READ-ONLY AUDIT；canonical path 为 `03_implementation-roadmap/milestones/M9/机票筛选Agent_M9_CTRIP_Candidate_Pipeline_Compatibility_Audit_Unit_Specification_V1.0.docx`。该 Unit 以上游 CTRIP Mapper 与 CTRIP Canonical Entry authority 为前置事实，仅授权对 existing M4 Candidate Merger / CandidateSnapshot path 是否兼容 CTRIP canonical outputs 进行只读审计。Audit execution 默认 NO SOURCE CHANGE / NO TEST CHANGE / NO FIXTURE CHANGE，且 NO COMMIT / NO PUSH。该登记不授权 Candidate Merger implementation、CandidateSnapshot implementation/schema change、multi-provider aggregation、provider registry / Composition Root、Ranking、Recommendation、Purchase Guidance、Public API、DB/migration、CI gate / Golden output 或 Shared Canonical Mapping Specification 修改。Shared Contract Amendment：NONE；只有 read-only audit evidence 证明真实 shared gap 时，才进入 STOP / Human Decision / Contract Amendment 流程。

M9-CTRIP-CANDIDATE-INTEGRATION-U1 CTRIP Candidate Pipeline Integration Unit Specification V1.0 已登记为 formal Implementation Unit Specification / Contract Authority。Unit status：PASS / IMPLEMENTED at commit `0e5e052de5bc87207bb5d1760217dddb245f4bad`；canonical path 为 `03_implementation-roadmap/milestones/M9/机票筛选Agent_M9_CTRIP_Candidate_Integration_Unit_Specification_V1.0.docx`。该 Unit 接受上游 Audit = PASS WITH LOCAL INTEGRATION GAP，确认 Existing CandidateMerger / CandidateSnapshot semantics sufficient、Provider-local integration only sufficient、Redundant CTRIP-specific merger/snapshot semantics needed = NO、Contract Amendment = NONE。该 Unit 仅授权并实现 CTRIP single-provider candidate wiring，通过 provider-local / application wiring 复用 existing CandidateMerger 与 existing CandidateSnapshotAssembler；未授权也未实现 shared merger/snapshot semantic change、FLIGGY + CTRIP multi-provider aggregation、Ranking、Recommendation、Public API、Purchase Guidance、Domain/Provider Port/schema/frontend change、live CTRIP acquisition、anti-bot bypass、cookie/token/HAR persistence 或 M9 final Provider selection。

M9-CTRIP-RUNTIME-INTEGRATION-AUDIT-U1 CTRIP Runtime Integration Compatibility Audit Unit Specification V1.0 已登记为 formal Read-Only Runtime Integration Audit Unit Specification / Authority。Unit status：AUTHORIZED / READY FOR READ-ONLY AUDIT；canonical path 为 `03_implementation-roadmap/milestones/M9/机票筛选Agent_M9_CTRIP_Runtime_Integration_Audit_Unit_Specification_V1.0.docx`。该 Unit 以上游 CTRIP acquisition/browser authorities、CTRIP Mapper、CTRIP Canonical Entry 与已 PASS 的 CTRIP Candidate Integration 为前置事实，仅授权对 runtime/application/provider wiring 进行只读审计：SearchPlan → CTRIP provider execution、Browser acquisition / sanitized evidence handoff、mapper/canonical/candidate callable seams、provider registry/composition extension point 与 provider execution outcome semantics。Audit execution 默认 NO SOURCE CHANGE / NO TEST CHANGE / NO FIXTURE CHANGE / NO LIVE CTRIP；不授权 runtime implementation、SearchPlan / FlightProvider Port / ProviderStatus / Execution semantic change、browser acquisition implementation change、registry/composition change、Candidate pipeline change、API/frontend/DB/CI change、live CTRIP call、anti-bot bypass 或 Contract Amendment。Contract Amendment：NONE unless later read-only audit evidence proves a real shared gap and enters STOP / Human Decision。

M9-CTRIP-RUNTIME-INTEGRATION-U1 CTRIP Runtime Integration Unit Specification V1.0 已登记为 formal Implementation Unit Specification / Contract Authority。Unit status：AUTHORIZED / READY FOR IMPLEMENTATION；canonical path 为 `03_implementation-roadmap/milestones/M9/机票筛选Agent_M9_CTRIP_Runtime_Integration_Unit_Specification_V1.0.docx`。该 Unit 接受上游 Runtime Integration Audit 结论 `PASS WITH LOCAL RUNTIME WIRING GAP`，确认 shared/generic runtime pipeline sufficient、provider-local runtime wiring only sufficient = YES、Shared Contract Amendment = NONE。该 Unit 仅授权 CTRIP provider-local `FlightProvider` execution adapter 与 additive bootstrap/provider registry/composition wiring；必须复用 existing SearchPlan / FlightProvider Port / ProviderSearchResult and provider status semantics / M7 lifecycle and cancellation ownership / Browser acquisition and sanitizer / CtripProviderMapper / CTRIP Canonical Entry / Candidate Integration path。该 Unit 不授权 SearchPlan / FlightProvider Port / ProviderStatus / M7 lifecycle shared semantic change、CandidateMerger / CandidateSnapshot / canonical identity change、FLIGGY + CTRIP multi-provider aggregation、provider selection/ranking policy change、Ranking、Recommendation、Purchase Guidance、Public API、frontend、database/schema change、live CTRIP as ordinary validation、anti-bot/challenge bypass、internal endpoint replay、cookie/session/token/HAR persistence 或 M9 final Provider selection。Bounded live revalidation remains separate future unit。Contract Amendment：NONE。
