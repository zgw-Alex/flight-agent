...
4.  **当前 Milestone / Implementation Unit 已确认规格**
    ...
    -   `03_implementation-roadmap/milestones/M8/机票筛选Agent_M8-U6H-CA04-R1_Post-Closure_Resolver_Eligibility_Candidate_Coverage_Repair_Specification_V0.1.docx`
        ...
        -   R1 Decision Status：AUTHORIZED；Implementation Status：NOT IMPLEMENTED。
        ...
        -   M3 Requirement authority、M6 ranking authority / weights / formula、M7 semantic diff / impact / execution authority 保持不变；R1 不授权 new semantic vocabulary、new scopes、free numeric weights、3+ preference ordering、`DEPARTURE_TIME` / `ARRIVAL_TIME` / `AIRPORT_MATCH` ranking expansion、conditional tradeoff IR、utility modeling 或 direct LLM commit authority。当前 bounded repair scope 下 Contract Amendment required：NO；若实现需要新增语义或 authority，必须 STOP 并进入单独 Contract Amendment。
...
M8-U6H-CA04-R1 Post-Closure Resolver Eligibility Candidate Coverage Repair Specification V0.1 已登记为 accepted additive post-closure repair authority。R1 仅修复 post-closure 24-case batch 暴露的 CA04 runtime reachability 缺口，不重新打开 CA04，不替代 CA04 base Specification、CA04-CA01 amendment 或 CA04 formal closure。R1 授权范围限于现有 PRICE / FEWER_STOPS importance semantics 的 Parser resolver eligibility / candidate coverage、CA04 已授权的 binary PRICE-vs-FEWER_STOPS relative routing、CA04-CA01 explicit no-preference removal 的 Patch routing，以及 unsupported semantic residue propagation；R1 implementation 尚未执行。R1 不授权 new semantic vocabulary、new scopes、free numeric weights、3+ preference ordering、DEPARTURE_TIME / ARRIVAL_TIME / AIRPORT_MATCH ranking expansion、conditional tradeoff IR、utility modeling 或 direct LLM commit authority；M3/M6/M7 authority 保持不变；bounded repair scope 下不需要新的 Contract Amendment。
