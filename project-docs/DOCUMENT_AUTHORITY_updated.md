...
4.  **当前 Milestone / Implementation Unit 已确认规格**
    ...
    -   `03_implementation-roadmap/milestones/M8/机票筛选Agent_M8-U6H-CA04-CA01_Explicit_No-Preference_Removal_Contract_Amendment_Specification_V0.1.docx`
        ...
        -   CA01 Decision Status：ACCEPTED；Implementation Authority：ENABLED FOR CA04-U3 RESUME；Implementation Status：NOT IMPLEMENTED BY INTAKE。
        ...
        -   CA01 不把 `importance=None`、`LOW` 或 low confidence 解释为 removal，不影响 hard `MAX_PRICE` / `MAX_STOPS`，不改变 M3 Requirement authority、M6 ranking weights/formula 或 M7 stale/impact/execution authority；不授权 Prompt V3、live DeepSeek behavior、新 ranking scopes、free numeric weights、conditional tradeoff、utility IR 或 direct LLM commit authority。CA04-U3 可在本治理登记后恢复，但本 intake 不实现 CA04-U3。
