...
def main() -> int:
        ...
        PromptRenderRequest(
            ...
            build_initial_requirement_prompt_context(
                InitialRequirementInterpretationRequest(
                    user_message=(
                        "Find a synthetic test flight from Beijing to Shanghai tomorrow morning "
                        "under CNY 1200."
                    ),
                    locale="en-US",
                )
    ...
    minimal_result = transport.invoke(
        _smoke_request(settings, model_id, rendered_prompt, "deepseek-minimal-smoke"),
        timeout_seconds=settings.deepseek_timeout_seconds,
    )
    result = LLMInvocationRuntime(transport).invoke(
        _smoke_request(settings, model_id, rendered_prompt, "deepseek-structured-smoke")
    )
    print(f"minimal Chat Completion: {'PASS' if minimal_result.output_text else 'FAIL'}")
    ...
    if result.failure is not None:
        print(f"structured failure code: {result.failure.code}")
...
def _smoke_request(
    ...
    return LLMInvocationRequest(
        invocation_id=LLMInvocationId(f"{invocation_prefix}-{uuid4()}"),
