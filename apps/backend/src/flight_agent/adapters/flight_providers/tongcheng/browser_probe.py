"""Probe-local TONGCHENG browser evidence acquisition.

This module intentionally stays outside the formal FlightProvider adapter path.
It produces provider-side raw evidence for M9-TONGCHENG-BROWSER-PROBE-U1 and does not
...
TONGCHENG_BROWSER_PROBE_VERSION = "m9-tongcheng-browser-probe-u1-v1.0"
TONGCHENG_PROVIDER_ID = "TONGCHENG"
TONGCHENG_ACQUISITION_MODE = "BROWSER"
_TONGCHENG_FLIGHT_ENTRY_URL = "https://www.ly.com/flights/home"
...
class TongchengProbeInput:
    ...
    headless: bool = True
    entry_url: str = _TONGCHENG_FLIGHT_ENTRY_URL

    def __post_init__(self) -> None:
        if not self.origin_text.strip():
            raise ValueError("TongchengProbeInput origin_text is required")
        if not self.destination_text.strip():
            raise ValueError("TongchengProbeInput destination_text is required")
        if self.overall_deadline_seconds <= 0:
            raise ValueError("TongchengProbeInput overall_deadline_seconds must be positive")
...
@dataclass(frozen=True)
class TongchengLevel2OfferEvidence:
    evidence_index: int
...
@dataclass(frozen=True)
class TongchengDomCrossCheck:
    structured_evidence_index: int
    flight_identity_match: bool
    carrier_match: bool
    departure_time_match: bool
    arrival_time_match: bool
    airport_match: bool
    summary_price_match: bool
    diagnostic: str

    def is_representative_match(self) -> bool:
        return self.flight_identity_match and (
            self.departure_time_match or self.arrival_time_match or self.airport_match
        )

    def to_dict(self) -> dict[str, Any]:
        return {
            "structured_evidence_index": self.structured_evidence_index,
            "flight_identity_match": self.flight_identity_match,
            "carrier_match": self.carrier_match,
            "departure_time_match": self.departure_time_match,
            "arrival_time_match": self.arrival_time_match,
            "airport_match": self.airport_match,
            "summary_price_match": self.summary_price_match,
            "representative_match": self.is_representative_match(),
            "diagnostic": self.diagnostic,
        }


@dataclass(frozen=True)
class TongchengProbeRunResult:
    ...
    level1_evidence: tuple[TongchengLevel1Evidence, ...]
    level2_offer_evidence: tuple[TongchengLevel2OfferEvidence, ...]
    dom_cross_check: TongchengDomCrossCheck | None
        ...
        return sanitize_probe_payload(
            {
                ...
                "dom_cross_check": self.dom_cross_check.to_dict() if self.dom_cross_check else None,
...
def classify_tongcheng_result_state(
    *, html: str, level1_count: int = 0, level2_count: int = 0, timed_out: bool = False
    ...
    detector = summarize_tongcheng_detector_state(html)
...
def summarize_tongcheng_detector_state(html: str) -> dict[str, bool]:
    ...
    return {
        ...
        "result_container": has_flight_identity and has_price and has_selection_action,
...
) -> tuple[TongchengLevel1Evidence, ...]:
    evidence: list[TongchengLevel1Evidence] = []
    for captured in payloads:
        for itinerary_path, itinerary in _iter_dicts_with_any_key(
            captured.payload,
            ("flightNo", "flightNumber", "FlightNo", "airCompanyCode", "airCompanyName"),
        ):
            ...
            price_path, price_list = _find_first_value_by_keys(
                itinerary,
                (
                    "AirLowestPrice",
                    "HourLowestPrice",
            ...
            evidence.append(
                TongchengLevel1Evidence(
                    ...
                    itinerary_id=_field_from_keys(
                        itinerary,
                        ("itineraryId", "itineraryID", "routeKey", "sequenceNo", "flightNo"),
                        itinerary_path,
                    ),
                    flight_no=_field_from_keys(
                        segment,
                    ...
                    market_airline_code=_field_from_keys(
                        segment,
                        ("airCompanyCode", "marketAirlineCode", "airlineCode"),
                        segment_path or itinerary_path,
                    ),
                    market_airline_name=_field_from_keys(
                        segment,
                        ("airCompanyName", "marketAirlineName", "airlineName"),
                        segment_path or itinerary_path,
                    ),
                    ...
                    departure_datetime=_field_from_keys(
                        segment,
                        ("flyOffTime", "flyOffOnlyTime", "departureDateTime", "departureTime", "departDateTime"),
                        segment_path or itinerary_path,
                    ),
                    arrival_datetime=_field_from_keys(
                        segment,
                        ("arrivalTime", "arrivalOnlyTime", "arrivalDateTime", "arriveDateTime"),
                        segment_path or itinerary_path,
                    ),
                    ...
                    aircraft=_field_from_keys(
                        segment,
                        ("planeType", "aircraft", "aircraftName", "aircraftCode"),
                        segment_path or itinerary_path,
                    ),
                    duration=_field_from_keys(
                        segment,
                        ("duration", "durationMinutes", "flightDuration", "flyTime"),
                        segment_path or itinerary_path,
                    ),
...
) -> tuple[TongchengLevel2OfferEvidence, ...]:
    evidence: list[TongchengLevel2OfferEvidence] = []
    for captured in payloads:
...
def cross_check_structured_evidence_with_dom(
    level1_evidence: tuple[TongchengLevel1Evidence, ...] | list[TongchengLevel1Evidence],
    html: str,
) -> TongchengDomCrossCheck | None:
    if not level1_evidence or not html.strip():
        return None
    text = parse_html_text(html)
    item = level1_evidence[0]
    flight_no = _string_or_none(item.flight_no.raw_value)
    carrier = _string_or_none(item.market_airline_name.raw_value)
    departure = _time_fragment(item.departure_datetime.raw_value)
    arrival = _time_fragment(item.arrival_datetime.raw_value)
    departure_airport = _string_or_none(item.departure_airport.raw_value)
    arrival_airport = _string_or_none(item.arrival_airport.raw_value)
    price = _price_fragment(item.price_list.raw_value)
    return TongchengDomCrossCheck(
        structured_evidence_index=item.evidence_index,
        flight_identity_match=bool(flight_no and flight_no in text),
        carrier_match=bool(carrier and carrier in text),
        departure_time_match=bool(departure and departure in text),
        arrival_time_match=bool(arrival and arrival in text),
        airport_match=bool(
            (departure_airport and departure_airport in text)
            or (arrival_airport and arrival_airport in text)
        ),
        summary_price_match=bool(price and price in text),
        diagnostic="provider-local XHR-to-DOM cross-check; no canonical mapping",
    )


async def run_tongcheng_browser_probe(probe_input: TongchengProbeInput) -> TongchengProbeRunResult:
    """Run the opt-in live TONGCHENG browser probe without persisting browser session state."""
    ...
    try:
        ...
        raise RuntimeError("Playwright is required for the opt-in live TONGCHENG browser probe") from exc
    ...
    try:
        async with async_playwright() as playwright:
            ...
            await page.goto(
                _build_tongcheng_search_url(probe_input),
                wait_until="commit",
    ...
    diagnostics["dom_cross_check"] = dom_cross_check.to_dict() if dom_cross_check else None
    ...
    return TongchengProbeRunResult(
        provider_identity=TONGCHENG_PROVIDER_ID,
        ...
        sanitized_source_ref=_sanitize_source_ref(_build_tongcheng_search_url(probe_input)),
        level1_evidence=level1_evidence,
        level2_offer_evidence=level2_evidence,
        dom_cross_check=dom_cross_check,
        diagnostics=diagnostics,
    )


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Opt-in TONGCHENG bounded browser probe")
    ...
    parser.add_argument("--entry-url", default=_TONGCHENG_FLIGHT_ENTRY_URL)
    parser.add_argument("--output-json", type=Path)
    args = parser.parse_args(argv)

    result = asyncio.run(run_tongcheng_browser_probe(TongchengProbeInput.from_args(args)))
...
def _offer_evidence_from_dict(index: int, offer: dict[str, Any], path: str) -> TongchengLevel2OfferEvidence:
    return TongchengLevel2OfferEvidence(
...
def _offer_evidence_from_visible_text(visible_text: str) -> TongchengLevel2OfferEvidence | None:
    ...
    return TongchengLevel2OfferEvidence(
        ...
        seller_supplier=_field_from_text(normalized, ("同程", "供应商", "航司", "代理")),
        price=(
...
def _level1_mapping_feasibility(segment: dict[str, Any], price_list: Any) -> dict[str, str]:
    flight_no = _find_first_value_by_keys(segment, ("flightNo", "flightNumber", "FlightNo"))[0] is not None
    dep = (
        _find_first_value_by_keys(
            segment,
            (
                "flyOffAirportCode",
                "flyOffAirportName",
    ...
    times = (
        _find_first_value_by_keys(
            segment,
            ("flyOffTime", "flyOffOnlyTime", "arrivalTime", "arrivalOnlyTime", "departureDateTime"),
        )[0]
        is not None
    )
    return {
...
def _aggregate_mapping_feasibility(
    level1: tuple[TongchengLevel1Evidence, ...], level2: tuple[TongchengLevel2OfferEvidence, ...]
) -> dict[str, str]:
    return {
...
def _classify_response_url(url: str, *, level2_capture_enabled: bool) -> str | None:
    lowered = url.lower()
    if "getflightlist" in lowered:
        return "getflightlist"
    if level2_capture_enabled and any(
        token in lowered
        for token in ("h5multi", "price", "product", "shopping", "booking", "order", "rule", "recommend")
    ):
...
def _build_tongcheng_search_url(probe_input: TongchengProbeInput) -> str:
...
def _time_fragment(value: Any) -> str | None:
...
def _price_fragment(value: Any) -> str | None:
    text = json.dumps(value, ensure_ascii=False) if isinstance(value, dict | list) else _string_or_none(value)
    if text is None:
        return None
